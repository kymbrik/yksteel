...
<?php
		global $link;
			...
			foreach($ids as $productId){
				$goods = selectAllItems($productId['product_id']);
					...
					foreach($goods as $item){
						...
						if($item['attribute_name']=='Processor') array_push($arrayOfProcessorsNames,$item['attribute_value']);
			...
			$result = array_unique($arrayOfProcessorsNames);
...
?>
   ...
   <p>
		...
		<input type="checkbox" name="check_list[]" value="<?=$i?>" ><?=$result[$i]?></br>
		<?}?>
   
   </p>
   <p><input type="submit" name="filter" value="Отправить"></p>
  </form>
	...
	if($category == $searchTerm ||  $brand == $searchTerm){?>
				<figure id="<?=$product_id ?>" class="products">
					...
					<div class="figDescr">
						...
						<input class="hide"  type="button" value="Скрыть" onclick="hideDiv();">
						<input  class="show"  type="button" value="Показать" onclick="showDiv();">
					</div>
			...
			if(!isset($_POST['search']) or $_POST['searchBox']==""){
